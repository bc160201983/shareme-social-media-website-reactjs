...
const CreatePin = ({ user }) => {
  ...
  const savePin = async () => {
    if (title && about && destination && imageAsset && category) {
      ...
      setLoading(true);
      const dataRef = await addDoc(pinRef, data);

      const spaceRef = ref(storage, `pin/${dataRef.id}/image`);
      await uploadString(spaceRef, imageAsset, "data_url")
        .then(async () => {
          const downloadUrl = await getDownloadURL(spaceRef);
          await updateDoc(doc(db, "pins", dataRef.id), {
            image: downloadUrl,
          });
        })
        .catch((error) => {
